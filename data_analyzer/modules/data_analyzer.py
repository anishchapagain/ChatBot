...
class DataAnalyzer:
    ...
    def create_time_series_plot(self, df: pd.DataFrame, date_column: str, value_column: str) -> Optional[Dict]:
        ...
        try:
            ...
            valid_data = df.dropna(subset=[date_column, value_column])
            
            # Sort by date
            valid_data = valid_data.sort_values(by=date_column)
            
            return {
                "type": "line",
                "x": valid_data[date_column].dt.strftime("%Y-%m-%d").tolist(),
                "y": valid_data[value_column].tolist(),
                "date_column": date_column,
                "value_column": value_column
            }
        
        except Exception as e:
            self.error_handler.handle_error(e, "Error creating time series plot")
            return None
