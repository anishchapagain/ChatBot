...
class Dashboard:
    ...
    def _render_summary_statistics(self, df: pd.DataFrame):
        ...
        if not numerical_columns.empty:
            ...
            # Calculate mean absolute deviation manually (since pandas.DataFrame.mad() is removed in pandas 2.0+)
            mad_values = []
            for col in numerical_columns:
                # Calculate mean absolute deviation: mean(|x_i - mean(x)|)
                col_mean = df[col].mean()
                mad = (df[col] - col_mean).abs().mean()
    ...
    def _render_time_series_analysis(self, df: pd.DataFrame):
        ...
        date_columns = DateUtils.find_date_columns(df, self.error_handler)
        ...
        if selected_date_column and selected_value_column:
            ...
            df_copy, success, warning = DateUtils.convert_column_to_datetime(
                df_copy, selected_date_column, self.error_handler
            )
            ...
            if not success:
                st.error(f"Failed to convert column '{selected_date_column}' to date format. Please select a different column.")
                return
            
            # Filter out rows with invalid dates or values
            valid_data = df_copy.dropna(subset=[selected_date_column, selected_value_column])
            
            if len(valid_data) == 0:
                st.error("No valid data points after filtering null values.")
                return
                
            # Sort by date
            valid_data = valid_data.sort_values(by=selected_date_column)
            
            # Create time series plot
            fig = go.Figure(data=go.Scatter(
                x=valid_data[selected_date_column],
                y=valid_data[selected_value_column],
                mode='lines+markers'
            ))
            ...
            if analysis_type == "Raw Data":
                ...
                try:
                    ...
                    temp_df = valid_data[[selected_date_column, selected_value_column]].copy()
                        
                    # Calculate moving average
                    temp_df['moving_avg'] = temp_df[selected_value_column].rolling(window=window_size).mean()
                    ...
                    fig.add_trace(go.Scatter(
                        x=temp_df[selected_date_column],
                        y=temp_df[selected_value_column],
                        mode='lines',
                        name='Raw Data'
                    ))
            ...
            st.warning("Please select both date and value columns to create a time series analysis.")
